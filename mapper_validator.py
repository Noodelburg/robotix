#!/usr/bin/env python3
"""Validator for mapper.py outputs."""

import argparse
import json
from pathlib import Path

from prompts import build_mapper_validation_prompt
from mapper import (
    CATEGORIES,
...
def validate_mapper(input_json_path, output_json_path=None):
    ...
    if document and current_output is not None:
        audit_issues = merge_issues(
            audit_issues,
            audit_mapper_output(current_output, document),
        )
    ...
    if document and current_output is not None:
        context = build_known_context(document)
        prompt = build_mapper_validation_prompt(
    ...
    remaining_issues = []
    if document and current_output is not None:
        remaining_issues = audit_mapper_output(current_output, document)
    elif audit_issues:
